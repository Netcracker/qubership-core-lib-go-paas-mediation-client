...
> 1. `Route.Spec.StreamIdleTimeout` (explicit Gateway API duration, e.g., `"1800s"`, `"30m"`)
> 2. Legacy nginx annotations: `max(proxy-read-timeout, proxy-send-timeout)` (auto-converted to `streamIdleTimeout`)
>
> **Usage example:**
> ```go
> route := &entity.Route{
